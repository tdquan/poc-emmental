...
export default () => {
        ...
        dispatch(
          requestData({
            ...
            handleFail: (beforeState, action) =>
              resolve(
                requests(beforeState.requests, action)["/users/signin"].errors
              ),
            handleSuccess: (state, action) => {
              resolve();
              const nextUrl = from ? decodeURIComponent(from) : "/";
  ...
  return (
    <>
      <Header />
      <Main className="signin" withHeader>
        <section>
          <ReactFinalForm onSubmit={handleFormSubmit} render={Form} />
        </section>
      </Main>
    </>
  );
